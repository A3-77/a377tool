...
上传视频后如果超过档位时长上限（卡片循环是 8 秒），会弹出选段框：

- 一条全长轨道，上面是可拖的选区 —— 两端调长度、中间整体挪、点轨道空白直接跳过去
- 「▶ 试看这段」先确认截得对不对，光看两个数字判断不了
- **只在真的超长时才弹**，没超长不会打扰

原来是一律从头截 8 秒。那截的是开头，而想留的那段经常在中间 ——
等于替用户做了决定，用户既看不到截了哪段、也没法改。
判定模块本来就支持自定义区间（`analyze()` 里的 `o.trim`），缺的只是这个界面。
...
> 这里体积**变大**是正常的：重新编码后尺寸也变了（640×360 → 640×426），不是 bug。

## 验证

`tests/test_clip.mjs` 端到端 12 项，**本地和线上各跑一遍全过**：
上传 12 秒素材 → 弹框 → 拖到 30% → 拖右手柄改长度 → 真的处理完并给出反馈。

KV 里现在有 6 个文件（之前 0 个）—— 上传链路第一次被真正打通。
