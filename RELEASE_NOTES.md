...
原来超长视频一律从头截 8 秒。那截的是开头，而想留的那段经常在中间 ——
等于替用户做了决定，用户既看不到截了哪段、也没法改。

判定模块本来就支持自定义区间（`analyze()` 里的 `o.trim`），缺的只是个界面。
现在有两条路：

### 上传时：超过时长上限才问

一条全长轨道，上面是可拖的选区 —— 两端调长度、中间整体挪、点轨道空白直接跳过去。
「▶ 试看这段」先确认截得对不对，光看两个数字判断不了。
**只在真的超长时才弹**，没超长不会打扰。

### 已经在用的素材：随时能重新截取

已经传上去、已经在列表里的视频，操作区多了个「✂ 截取」：

```
抓回素材 → 探测时长 → 选段 → 处理 → 重新上传 → 换掉这一项的 src
```

这里是用户主动要改，所以**不管超不超时长上限都给选**，取消就什么都不做。

两条路共用一个选段框和同一套判定（`tests/test_clip.mjs` / `tests/test_reclip.mjs`）。

处理完的反馈能直接核对：

```
已上传 1 个，记得点保存（已处理 long12s.mp4：207 KB 12 秒 640×360 → 283 KB 6.9 秒 640×426）
已截取 0:00.3 – 0:08.3：处理完成，记得点保存
```

> 上传那条里体积**变大**是正常的：重新编码后尺寸也变了（640×360 → 640×426），不是 bug。

## 验证

- `tests/test_clip.mjs`（上传时选段）端到端 12 项
- `tests/test_reclip.mjs`（已有素材截取）端到端 8 项

**两个都是本地和线上各跑一遍全过。** KV 里现在有 6 个文件（之前 0 个）——
上传链路第一次被真正打通。
